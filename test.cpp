...
/*
    ...
    ! Defining contents of list upon creation makes list immutable
        ? superList *myList = new myList(<size>, <contents>); == immutable
        ? superList *myList = new myList(<size>); == mutable
        ? superList *myList = new myList(); == mutable
    ! Indexed doubly linked list
        * superList *myList = new myList(3,1,2,3);
            ? cout << temp[1];              --> 2
            ? cout << temp[1]->next;        --> 3
            ? cout << temp[1]->last;        --> 1
    ...
    ! Ordered and unordered insertion options both push and push_back style as well as bulk and indexed entry
        ...
        * superList *myList = new myList(3,1,3,5)
